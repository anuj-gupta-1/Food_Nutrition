#!/usr/bin/env python3
"""
Run trial classification using local Llama and display results
"""

import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__)))
from product_classifier import run_trial_classification
...
def display_product_result(result, index, total):
    ...
    if result.get('llm_response'):
        print(f"\n🤖 Llama Response: {result['llm_response'][:100]}...")
...
def display_results_by_confidence(results_df, high_conf, medium_conf, low_conf):
    ...
    print("\n" + "="*80)
    print("DISPLAYING LOCAL LLAMA RESULTS BY CONFIDENCE LEVEL")
    print("="*80)
    ...
    if avg_confidence >= 0.75:
        print("✅ Recommendation: Local Llama classification is working well")
    elif avg_confidence >= 0.65:
        print("⚠️  Recommendation: Results are acceptable, may need prompt tuning")
    else:
        print("❌ Recommendation: Llama needs prompt improvement")
    
    print(f"\n{'='*80}\n")


if __name__ == "__main__":
    # Run local Llama trial classification
    print("Starting Local Llama classification trial...")
    print("Using your local Ollama/Llama model...\n")
    ...
    if results_df is not None:
        ...
        print("❌ Trial run failed. Make sure Ollama is running: ollama serve")