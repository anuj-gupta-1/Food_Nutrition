...
SOURCE_FILTER = os.getenv("SOURCE_FILTER", None)  # e.g., "jiomart"
CATEGORY_WHITELIST = os.getenv("CATEGORY_WHITELIST", None)  # e.g., "dairy,snacks,beverages"
UPLOAD_LIMIT = int(os.getenv("UPLOAD_LIMIT", "0"))  # 0 = no limit
PER_CATEGORY_LIMIT = int(os.getenv("PER_CATEGORY_LIMIT", "0"))  # 0 = no per-category cap
...
def main():
    ...
    try:
        ...
        if CATEGORY_WHITELIST:
            cats = {c.strip().lower() for c in CATEGORY_WHITELIST.split(',') if c.strip()}
            df = df[df['category'].astype(str).str.lower().isin(cats)]
        # Optional limiting (per-category takes precedence if set)
        if PER_CATEGORY_LIMIT and PER_CATEGORY_LIMIT > 0 and 'category' in df.columns:
            df = (
                df.groupby(df['category'].astype(str), group_keys=False)
                  .head(PER_CATEGORY_LIMIT)
            )
        elif UPLOAD_LIMIT and UPLOAD_LIMIT > 0:
            df = df.head(UPLOAD_LIMIT)

        df = df.astype(str)  # Convert all to string for consistent handling
        print(f"Successfully prepared {len(df)} rows from {CSV_FILE_PATH}")
